...
The RTOS uses fixed-priority preemptive scheduling. The RTOS will always run
...
A project demonstrating the usage of the RTOS is set up in PlatformIO for an
STM32-Nucleo board (STM32F401RE) in *demo_project*.

The application has two worker tasks, prints when the timer expires, and prints
and flashes the on board LED when the button is pressed.
